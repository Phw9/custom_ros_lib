...
public:
    ...
    void LiDARBinCallBack(const livox_ros_driver2::CustomMsg::ConstPtr& msg) {
        sensor_msgs::PointCloud2 pc2;
        pc2.header = msg->header;
        ...
        // Save to BIN
        std::string bin_filename = generateBinFileName();
        pcl::io::savePCDFileBinary(bin_filename, pcl_cloud);

        // Save timestamp
        saveTimestamp(bin_filename, msg->header.stamp);

        count_++;
        ROS_INFO("Save %d.bin file. Time : %f", count_, msg->header.stamp.toSec());
    }
    ...
    std::string generateBinFileName() {
        std::ostringstream ss;
        std::string binDir = save_dir_ + "/bin/";
        ss << binDir << std::setw(5) << std::setfill('0') << count_ << ".bin";
        return ss.str();
    }
